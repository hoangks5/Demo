...
def get_coinmarketcap_price(symbol):
    ...
    header = {'Accepts': 'application/json', 'X-CMC_PRO_API_KEY' : '92addac5-8f79-4a21-8c26-d21511e48fbc'}
    time1 = time.time()
    response = requests.get(url=url,params=parameters,headers=header)
    price = response.json()['data'][symbol.split('-')[0]]['quote'][symbol.split('-')[1]]['price']
    avg = {
        'token' : symbol.split('-')[0],
...
while True:
    ...
    for test in dict_test:
        ...
        #threading_.append(threading.Thread(target=get_coinmarketcap_price,args={test,}))
